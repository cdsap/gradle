...
[[sec:command_line_concurrent_invocations]]
`--concurrent`, `--no-concurrent` incubating-label:[]::
Opt in to behavior tuned for <<dependency_caching.adoc#sec:cache-locking,several Gradle processes>> sharing the same <<directory_layout.adoc#dir:gradle_user_home,Gradle user home>> (for example agent-driven or scripted parallel invocations).
This option does not replace normal project/task parallelism; it is orthogonal to `--parallel`.
+
The same flag is available as the <<build_environment.adoc#sec:gradle_configuration_properties,Gradle property>> `org.gradle.concurrent`. _Default is off_.
+
When concurrent invocations are enabled, Gradle may emit additional lifecycle messages while waiting on shared <<dependency_caching.adoc#sec:cache-locking,user home>> file locks.
You can disable those messages with the internal property `org.gradle.internal.concurrent.lock-diagnostics=false` (unsupported; for advanced troubleshooting only).
+
link:https://scans.gradle.com/[Build scans] may list operations named like pass:[`Acquire file lock on ...`] with structured details for lock targets; the operation result can indicate whether lock acquisition observed contention.

...
If you intentionally run many Gradle clients against the same user home (for example from automation), consider the incubating <<command_line_interface.adoc#sec:command_line_concurrent_invocations,`--concurrent`>> option and related <<build_environment.adoc#sec:gradle_configuration_properties,Gradle properties>>.
